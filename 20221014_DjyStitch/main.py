...
#python ./20220916_DjyStitch/main.py
#选择拼接模式
stitch_mode='Z' #{‘XYZ’， ‘XY’， ‘Z'}
...
if stitch_mode=='XYZ':
    ...
    if_rename_file = True
    # 图片命名格式
    img_name_format=r'%s\%s_t%.4d_z%.4d_ch%.2d.%s'#%(img_path,img_name,z_dimension_ordinal,channel_ordinal,img_type)
    # 图片名
    img_name = 'Region'
    #中间文件储存路径
...
##################################################
    ...
    if_rename_file = True
    # 图片命名格式
    img_name_format = r'%s\%s_t%.4d_z%.4d_ch%.2d.%s'  # %(img_path,img_name,tile_ordinal,z_dimension_ordinal,channel_ordinal,img_type)
    # 图片名
    img_name = 'Region'
    # 中间文件储存路径
...
##################################################
    ...
    # 图片命名格式
    img_name_format = '%s'+os.sep+'%s_z%.4d_ch%.2d.%s'  # %(img_path,img_name,z_dimension_ordinal,channel_ordinal,img_type)
    # 图片名
    img_name = 'Region'
    # 中间文件储存路径
    info_IO_path = file_path
...
if __name__=='__main__':
    ...
    if stitch_mode=='Z':
        for i in range(0,layer_num-1):
            start_vertical_stit_merged(i,img_name_format,file_path,info_IO_path,file_name_format,img_name,channel_num,channel_ordinal,
                                       img_file_type,img_data_type,overlap_ratio=overlap_ratio,vert_step=step,
                                       pyr_down_times=pyr_down_times,if_median_blur=if_median_blur,
