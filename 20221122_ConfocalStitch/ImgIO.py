        ...
        for ch_th in range(ch_num):
            ...
            for k in range(tile_num):
                ...
                if x_th < 0 or x_th + dim_elem_num[0] > voxel_num[0] or y_th < 0 or y_th + dim_elem_num[1] > \
                        voxel_num[1]:
                    continue
                this_img[y_th:y_th + dim_elem_num[1], x_th:np.min((x_th+15, voxel_num[0]))] = 255
                this_img[y_th:y_th + dim_elem_num[1], x_th + dim_elem_num[0]:np.min((x_th + dim_elem_num[0]+15, voxel_num[0]))] = 255
                this_img[y_th:np.min((y_th+15, voxel_num[1])),x_th:x_th + dim_elem_num[0]] = 255
                this_img[y_th+dim_elem_num[1]:np.min((y_th+dim_elem_num[1]+15, voxel_num[1])), x_th:x_th + dim_elem_num[0]] = 255
            #
