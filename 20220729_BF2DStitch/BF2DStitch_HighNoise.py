...
def import_img(img_path,ordinal,dim_elem_num):
    ...
    for i in range(dim_elem_num[2]):
        img_name=r'%s\TileScan 1_s%.4d_z%.3d_RAW_ch00.tif'%(img_path,ordinal,i)
...
def choose_reference_tile(i,tile_contact_list,if_tile_stitched,times):
    '''
...
def run_sift_stitcher(lock,img_path,dim_elem_num,dim_len,voxel_len,tile_num,tile_field,tile_pos,tile_contact,
    ...
    while(False in if_tile_stitched):
        ...
        j=choose_reference_tile(i,tile_contact[i,:],if_tile_stitched,times)
        if j==-1:
...
#python C:\Users\dingj\ZhaoLab\20220729_BF2DStitch\BF2DStitch_HighNoise.py
