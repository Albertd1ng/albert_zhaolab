...
def get_img_npy_basic_info(layer_num, save_path):
    ...
    voxel_len - vector, float64, (3)
        the length of each dimension for a voxel.
    '''
    dim_elem_num = np.load(os.path.join(save_path,'dim_elem_num_%.4d.npy' % (layer_num)))
    dim_len = np.load(os.path.join(save_path,'dim_len_%.4d.npy' % (layer_num)))
    voxel_len = np.load(os.path.join(save_path,r'\voxel_len.npy'))
    return dim_elem_num, dim_len, voxel_len
...
def get_img_info_hori_stit(layer_num,save_path):
    ...
    '''
    tile_pos_stitch=np.load(os.path.join(save_path,'tile_pos_stitch_%.4d.npy'%(layer_num)))
    axis_range=np.load(os.path.join(save_path, 'axis_range_stitch_%.4d.npy' % (layer_num)))
    first_last_index=np.load(os.path.join(save_path,'first_last_index_%.4d.npy' % (layer_num)))
    return tile_pos_stitch,axis_range,first_last_index

def get_img_info_vert_stit(layer_num,save_path):
    tile_pos_stitch = np.load(os.path.join(save_path, 'tile_pos_zstitch_%.4d.npy' % (layer_num)))
    axis_range = np.load(os.path.join(save_path, 'axis_range_zstitch_%.4d.npy' % (layer_num)))
    first_last_index = np.load(os.path.join(save_path, 'first_last_index_zstitch%.4d.npy' % (layer_num)))
    return tile_pos_stitch, axis_range, first_last_index
...
def save_img_info(layer_num, save_path, dim_elem_num, dim_len, voxel_len, tile_num, tile_pos):
    ...
    '''
    np.save(os.path.join(save_path,'dim_elem_num_%.4d.npy' % (layer_num)), dim_elem_num)
    np.save(os.path.join(save_path,'dim_len_%.4d.npy' % (layer_num)), dim_len)
    np.save(os.path.join(save_path,'voxel_len.npy'), voxel_len)
    np.save(os.path.join(save_path,'tile_num_%.4d.npy' % (layer_num)), tile_num)
    np.save(os.path.join(save_path,'tile_pos_%.4d.npy' % (layer_num)), tile_pos)
...
def save_img_info_hori_stit(layer_num,save_path,tile_pos_stitch,axis_range,first_last_index):
    ...
    first_last_index -vector, uint32, (2)
    '''
    np.save(os.path.join(save_path,'tile_pos_stitch_%.4d.npy'%(layer_num)),tile_pos_stitch)
    np.save(os.path.join(save_path,'axis_range_stitch_%.4d.npy' % (layer_num)), axis_range)
    np.save(os.path.join(save_path,'first_last_index_%.4d.npy' % (layer_num)), first_last_index)

def save_img_info_vert_stit(layer_num,save_path,tile_pos,axis_range,first_last_index):
    np.save(os.path.join(save_path,'tile_pos_zstitch_%.4d.npy' % (layer_num)), tile_pos)
    np.save(os.path.join(save_path,'axis_range_zstitch_%.4d.npy' % (layer_num)), axis_range)
    np.save(os.path.join(save_path,'first_last_index_zstitch_%.4d.npy' % (layer_num)), first_last_index)

def save_img_info_vert_stit_merged(layer_num,save_path,dim_elem_num,axis_range,first_last_index):
    np.save(os.path.join(save_path,'dim_elem_num_zstitch_%.4d.npy' % (layer_num)), dim_elem_num)
    np.save(os.path.join(save_path,'axis_range_zstitch_%.4d.npy' % (layer_num)), axis_range)
    np.save(os.path.join(save_path,'first_last_index_zstitch_%.4d.npy' % (layer_num)), first_last_index)

def get_img_info_vert_stit_merged(layer_num,save_path):
    dim_elem_num=np.load(os.path.join(save_path,'dim_elem_num_zstitch_%.4d.npy' % (layer_num)))
    axis_range=np.load(os.path.join(save_path,'axis_range_zstitch_%.4d.npy' % (layer_num)))
    first_last_index=np.load(os.path.join(save_path,'first_last_index_zstitch_%.4d.npy' % (layer_num)))
