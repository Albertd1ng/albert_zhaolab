...
def adjust_contrast(img1, img2):
    ...
    if img_data_type=='uint8':
        img1, img2 = np.uint8(np.clip(m / m1 * img1, 0, 255)), np.uint8(np.clip(m / m2 * img1, 0, 255))
    elif img_data_type=='uint16':
        img1, img2 = np.uint16(np.clip(m / m1 * img1, 0, 65535)), np.uint16(np.clip(m / m2 * img1, 0, 65535))
    return img1, img2