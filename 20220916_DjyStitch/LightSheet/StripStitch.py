...
def in_which_layer(i, layer_id_dict):
    for j in range(len(layer_id_dict)):
...
def strip_stitch(lock, layer_id_dict, dir_path, img_name_format, id_dir_dict, strip_pos, strip_cont, xy_v_num, z_v_num,
                 ch_th, img_type, img_dtype, if_strip_stitched, if_strip_shelved, strip_refer_id, xyz_shift,
                 voxel_range, pyr_down_times, choose_num=10, z_depth=200):
    ...
    while False in if_strip_stitched:
        ...
        try:
            ...
            break
        print('%s is stitching %d.th strip with %d.th strip.' % (this_name, i, j))
        ...
        print('%s: current stitch is %d.th strip with %d.th strip,\nxyz_shift is (%d, %d, %d)'
            % (this_name, i, j, *xyz_shift_mean))


def start_multi_strip_stitches(txt_path, save_txt_path, dir_path, img_name_format, channel_num, ch_th, img_type,
                               voxel_range=[50, 100, 0], pyr_down_times=2, choose_num=10, z_depth=200):
    ...
    process_num = 10
    print('Current processing quantity: %d' % process_num)
    process_list = []
    for i in range(process_num):
        one_pro = Process(target=strip_stitch, args=(
            lock, layer_id_dict, dir_path, img_name_format, id_dir_dict, strip_pos, strip_cont, xy_v_num, z_v_num,
            ch_th, img_type, img_dtype, if_strip_stitched, if_strip_shelved, strip_refer_id, xyz_shift,
    ...
    for i in process_list:
        i.join()
    print('start calculating new position!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    strip_pos_update = update_pos_after_shift(strip_pos, np.array(xyz_shift, dtype='int64').reshape(-1, 3),
                                              np.array(strip_refer_id, dtype='int64'))
