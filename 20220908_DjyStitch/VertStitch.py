...
def start_vertical_stit_merged(i, img_path_format,img_path, info_IO_path, file_name_format, img_name, channel_ordinal, img_type,
                          img_data_type,overlap_ratio,vert_step,pyr_down_times,if_median_blur,blur_kernel_size):
    if img_data_type=='uint8':
        img_mode=cv2.IMREAD_GRAYSCALE
    elif img_data_type=='uint16':
        img_mode=cv2.IMREAD_UNCHANGED
    ...
    dim_elem_num1[2], dim_elem_num2[2] = len(os.listdir(img_path1)),len(os.listdir(img_path2))
    first_last_index1[1],first_last_index2[1]=dim_elem_num1[2]-1,dim_elem_num2[2]-1
    img1=import_img_2D(file_name_format,img_path1,img_name,dim_elem_num1[2]-1,channel_ordinal,img_type=img_type,
    ...
    axis_range2 = update_lower_layer_info_merged(xy_shift_min,axis_range1,dim_elem_num2)
    print('start saving data')
