...
def import_img(img_path,file_name,ordinal,dim_elem_num):
    '''
    ...
    for i in range(dim_elem_num[2]):
        img_name=r'%s\%s_s%.4d_z%.3d_RAW_ch00.tif'%(img_path,file_name,ordinal,i)
...
def calculate_xyz_shift(i,index_j,border_list,img_path,file_name,dim_elem_num,voxel_range,step):
    '''
    ...
    #step - int 
    '''
    img1=np.float32(cv2.medianBlur(import_img(img_path,file_name,i,dim_elem_num),5))
    m1=np.mean(img1)
    if m1<0.001:
        return [0,0,0],0
    img2_list=[]
    for j in index_j:
        img2_list.append(np.float32(cv2.medianBlur(import_img(img_path,file_name,j,dim_elem_num),5)))
...
def choose_start_img(img_path,file_name,tile_num,dim_elem_num,previous=0):
    '''
    ...
    while(m<10 or abs(a-previous)<0.1*round(tile_num)):
        a=random.randint(int(0.1*tile_num),int(0.5*tile_num))
        m=np.mean(import_img(img_path,file_name,a,dim_elem_num))
    print('start_tile is %d.th tile, average is %.8f'%(a,m))
    return a

def import_2D_img(img_path,file_name,ordinal,dim_elem_num,z_th):
    img_name=r'%s\%s_s%.4d_z%.3d_RAW_ch00.tif'%(img_path,file_name,ordinal,z_th)
...
def calculate_axis_range_voxel_num(tile_pos,dim_elem_num,voxel_len):
    ...
    voxel_num=np.uint64(np.round((axis_range[:,1]-axis_range[:,0])/voxel_len))
...
def run_sift_stitcher(lock,img_path,file_name,dim_elem_num,dim_len,voxel_len,tile_num,tile_contact,tile_pos,
    ...
    while(False in if_tile_stitched):
        ...
        xyz_shift,loss_min=calculate_xyz_shift(i,index_j,border_list,img_path,file_name,dim_elem_num,voxel_range,step)
        lock.acquire()
        if_tile_being_stitched[i]=False
        if loss_min>1.5+0.25*if_tile_bad[i]:
...
def start_multi_stitchers(xml_path,img_path,file_name,save_path,layer_num,voxel_range=[21,21,9],step=3):
    dim_elem_num,dim_len,voxel_len,tile_num,tile_field,tile_pos=get_img_xml_info(xml_path,save_path)
    try:
    ...
    start_index=choose_start_img(img_path,file_name,tile_num,dim_elem_num,0)
    ...
    for i in range(process_num):
        one_pro=multiprocessing.Process(target=run_sift_stitcher,
                                        args=(lock,img_path,file_name,dim_elem_num,dim_len,voxel_len,tile_num,tile_contact,tile_pos,
                                              tile_pos_stitch,if_tile_stitched,if_tile_shelved,if_tile_bad,if_tile_being_stitched,
                                              voxel_range,step))
        one_pro.start()
    ...
    np.save(save_path+r'\axis_range_%.4d.npy'%(layer_num),axis_range)
...
#python C:\Users\dingj\ZhaoLab\20220803_BFhoriSIFTvertStitch\BFHorizontalStitch_HighNoise.py
if __name__=='__main__':
    ##############################
    #must be changed before starting program
    file_name='TileScan 1'
    xml_path=r'F:\Whole_0003\MetaData\TileScan 1.xml'
    img_path=r'F:\Whole_0003'
    save_path=r'C:\Users\dingj\ZhaoLab\20220803_BFhoriSIFTvertStitch'
    layer_num=3
    #############################
    start_multi_stitchers(xml_path,img_path,file_name,save_path,layer_num)